...
function AffiliateSidebar({ active, onNavigate, collapsed, setCollapsed, onLogout }) {
  return (
    <aside className="flex flex-col h-screen sticky top-0 transition-all duration-300 flex-shrink-0"
      ...
      {/* Logo */}
      <div className="flex items-center justify-between px-4 py-4 flex-shrink-0" style={{ borderBottom: '1px solid var(--border-0)' }}>
        {!collapsed ? (
      ...
      {!collapsed && (
        <div className="px-3 py-3 flex-shrink-0" style={{ borderBottom: '1px solid var(--border-0)' }}>
          <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl" style={{ background: 'rgba(16,185,129,0.06)' }}>
      ...
      <nav className="flex-1 overflow-y-auto px-2.5 py-3 space-y-4">
        {NAV_GROUPS.map(group => (
          <div key={group.title}>
            {!collapsed && (
              <div className="text-xs font-bold uppercase tracking-widest px-2 mb-1.5" style={{ color: 'var(--text-4)' }}>
                {group.title}
              </div>
            )}
...
function AffiliateHeader({ section, onNavigate }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between px-6 py-3 gap-4 flex-shrink-0"
      ...
      <div className="flex items-center gap-2">
        {/* Quick stats */}
        <div className="hidden lg:flex items-center gap-4 px-4 py-1.5 rounded-xl" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-0)' }}>
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-deep)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <AffiliateHeader section={section} onNavigate={setSection} />
