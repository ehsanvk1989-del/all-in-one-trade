...
import {
  ...
  Settings as SettingsIcon, LogOut, Bell, Search, ChevronLeft, ChevronRight,
  ShieldCheck, Database, Award, Sun, Moon,
} from 'lucide-react';
...
function AdminSidebar({ active, onNavigate, collapsed, setCollapsed, onLogout, theme }) {
              ...
              {group.items.map(item => {
                ...
                return (
                  <button key={item.id} onClick={() => onNavigate(item.id)}
                    ...
                    onMouseEnter={e => {
                      if (!isActive) {
                        e.currentTarget.style.background = isLight ? 'rgba(59,130,246,0.06)' : 'rgba(255,255,255,0.05)';
...
function AdminHeader({ section, theme, onToggleTheme }) {
    ...
    <header className="sticky top-0 z-20 flex items-center justify-between px-6 py-3 gap-4"
      ...
      <div className="flex items-center gap-2">
        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg"
          ...
          <input placeholder="Search…" className="bg-transparent outline-none text-xs w-32"
        ...
        <button onClick={onToggleTheme} title={isLight ? 'Switch to Dark Mode' : 'Switch to Light Mode'}
          ...
          style={{
            background: isLight ? 'rgba(59,130,246,0.09)' : 'var(--bg-surface)',
            border: `1px solid ${isLight ? 'rgba(59,130,246,0.22)' : 'var(--border-0)'}`,
            color: isLight ? '#3B82F6' : 'var(--text-3)',
          }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(59,130,246,0.35)'; e.currentTarget.style.color = '#3B82F6'; }}
          onMouseLeave={e => {
            e.currentTarget.style.borderColor = isLight ? 'rgba(59,130,246,0.22)' : 'var(--border-0)';
            e.currentTarget.style.color = isLight ? '#3B82F6' : 'var(--text-3)';
          }}>
        ...
        <div className="flex items-center gap-2 pl-1 pr-3 py-1 rounded-lg"
          style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-0)' }}>
          <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-black"
            style={{ background: 'linear-gradient(135deg, #3B82F6, #7C3AED)' }}>A</div>
...
export default function AdminApp() {
  ...
  const [theme, setTheme]       = useState(() => localStorage.getItem('pt_admin_theme') || 'dark');
  ...
  const toggleTheme  = () => {
    ...
    setTheme(next);
    localStorage.setItem('pt_admin_theme', next);
  };
  ...
  return (
    <div data-admin-theme={theme} className="flex h-screen overflow-hidden"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <AdminHeader section={section} theme={theme} onToggleTheme={toggleTheme} />
